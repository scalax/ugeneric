...
class RepContext extends Context6[RepNode] {
  override def append[X1, X2, X3, X4, X5, X6, Y1, Y2, Y3, Y4, Y5, Y6, Z1, Z2, Z3, Z4, Z5, Z6](x: RepNode[X1, X2, X3, X4, X5, X6], y: RepNode[Y1, Y2, Y3, Y4, Y5, Y6])(
    ...
    override def node(rep: Z6, l: List[Node]): List[Node]     = x.node(p.takeHead6(rep), y.node(p.takeTail6(rep), l))
    override def fieldPlus(data: Z4, l: List[Any]): List[Any] = x.fieldPlus(p.takeHead4(data), y.fieldPlus(p.takeTail4(data), l))
    override def fieldTail(l: List[Any]): (Z4, List[Any]) = {
      val x2 = x.fieldTail(l)
      val y2 = y.fieldTail(x2._2)
      (p.plus4(x2._1, y2._1), y2._2)
    }
    override def encodeRef(rep: Z6, path: Node, index: Int): (Z5, Int) = {
      val y2 = y.encodeRef(p.takeTail6(rep), path, index)
      val x2 = x.encodeRef(p.takeHead6(rep), path, y2._2)
      (p.plus5(x2._1, y2._1), x2._2)
    }
